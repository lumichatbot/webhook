...
def make_sfc_intent(return_entities=False):
    ...
    sentence_entities, _ = sample_entailing_endpoints(
        sentence_entities, hypothesis_entities
    )
    sentence_entities, _ = sample_conflicting_chaining(
        sentence_entities, hypothesis_entities
    )
...
def make_acl_intent(return_entities=False):
    ...
    sentence_entities, _ = sample_conflicting_rules(
        sentence_entities, hypothesis_entities
    )
...
def make_temporal_intent(return_entities=False):
    ...
    sentence_entities, _ = sample_entailing_timeranges(
        sentence_entities, hypothesis_entities
    )
...
def make_mixed_intent():
    ...
    if option == 0:
        ...
        sentence_entities, _ = sample_entailing_rules(
            sentence_entities, hypothesis_entities
        )
    elif option == 2:
        sentence_entities, _ = sample_entailing_qos(
...
def make_negation_entailment():
    ...
    if (
        "allow" in sentence_entities["operations"]
        or "block" in sentence_entities["operations"]
    ):
        if randint(0, 2) % 2 == 0:
