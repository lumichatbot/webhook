...
from anytree import AnyNode, find_by_attr, findall
...
def make_link(
    source, target, capacity, src_id=None, dst_id=None, src_port=0, dst_port=0
):
    ...
    return {
        ...
        "cost": 1.000,
        "properties": {"capacity": [capacity, "mbps"]},
    }
...
def write_dot():
    ...
    for i in range(1, 3):
        ...
        links.append(
            make_link(
                ...
                10000,
                "s1",
                core_switch["properties"]["name"],
                ports["s1"],
                ports[core_switch["properties"]["name"]],
            )
        )
        links.append(
            make_link(
        ...
        for j in range(1, 3):
            ...
            links.append(
                make_link(
                    ...
                    1000,
                    core_switch["properties"]["name"],
                    aggr_switch["properties"]["name"],
                    ports[core_switch["properties"]["name"]],
                    ports[aggr_switch["properties"]["name"]],
                )
            )
            links.append(
                make_link(
            ...
            for k in range(1, 5):
                ...
                links.append(
                    make_link(
                        ...
                        100,
                        aggr_switch["properties"]["name"],
                        edge_switch["properties"]["name"],
                        ports[aggr_switch["properties"]["name"]],
                        ports[edge_switch["properties"]["name"]],
                    )
                )
                links.append(
                    make_link(
    ...
    for middlebox in DATASET_MIDDLEBOXES:
        ...
        links.append(
            make_link("19.16.1.1", m["id"], 100, "s1", middlebox_name, ports["s1"])
        )
        links.append(
            make_link(m["id"], "19.16.1.1", 100, middlebox_name, "s1", 0, ports["s1"])
        )
        ports["s1"] += 1
    ...
    for l in links:
        ...
        dst_id = l["target_id"]
        str_writer.write(
            "\t{} -> {} [src_port={}, dst_port={}, cost=1];\n".format(
                src_id, dst_id, l["source_port"], l["target_port"]
            )
        )
        ...
        "nodes": [
            {
                ...
                "properties": {
                    "hostname": "gateway.rs.edu",
                    "handles": ["gateway", "internet", "network"],
                },
            }
        ],
        "links": [],
...
def is_descendent(parent, child):
    ...
    for node in parent_node.descendants:
        if "." in child:
            ...
            decendency = (
                next((True for x in node.properties["handles"] if x == child), False)
                or decendency
            )
...
def get_node_tree():
    ...
    for link in topology["links"]:
        ...
        if not parent:
            parent_node = next(
                (x for x in topology["nodes"] if x["id"] == parent_ip), None
            )
            parent = AnyNode(
                id=parent_node["id"],
        ...
        if not child:
            child_node = next(
                (x for x in topology["nodes"] if x["id"] == child_ip), None
            )
            child = AnyNode(
                id=child_node["id"],
                parent=parent,
                label=child_node["label"],
                properties=child_node["properties"],
                capacity=min(parent.capacity, link_capacity),
            )
