...
from alt_features import get_alt_features
            ...
            for j in range(i + 1, len(intents)):
                ...
                if sentence != hypothesis:
                    results.append((uni_stn, uni_hyp, text_stn, text_hyp, sentence, hypothesis,
                                    model.predict([get_alt_features(sentence, hypothesis)])))
...
def test(dataset_size, model_type):
    ...
    with open(config.CONTRADICTIONS_RESULTS_PATH.format(dataset_size, model_type), 'wb') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=',')
...
def learning_curve(dataset_size, model_type):
    ...
    with open(config.LEARNING_CURVE_PATH.format(dataset_size, model_type), 'wb') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=',')
...
def run():
    ...
    with open(config.CONTRADICTIONS_RESULTS_PATH.format('summary', '0'), 'wb') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=',')
        csv_writer.writerow(['dataset', 'model', 'k-fold', 'fit time', 'validation time', 'precision', 'recall', 'f1'])
        for dataset_size in dataset.DATASET_SIZES:
