...
from ..ambiguities import inspector
...
def build_accepted(request):
    ...
    if amibiguity:
        text = """
            The intent you described is possibly ambiguous with a previous one.
            Do you want to deploy it anyway or remove the old one?
        """
        return make_card_response(
            "Possible amibiguity ",
            text,
            text,
            beautify_intent(amibiguity["nile"]),
            suggestions=[
                "Deploy it anyway",
                "Remove old one and deploy new",
                "Keep old one",
            ],
        )
...
def feedback_train(request):
    ...
    if merlin_program:
        ...
        return make_simple_response(
            "Okay! Feedback received. Intent compiled and deployed!"
        )
