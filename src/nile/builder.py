...
def slot_filling(entities):
    ...
    if (
        "middleboxes" in entities
        and "add" not in entities["operations"]
        and "remove" not in entities["operations"]
    ):
        entities["operations"].append("add")

    if (
        "qos" in entities
        and "set" not in entities["operations"]
        and "unset" not in entities["operations"]
    ):
    ...
    if not entities["operations"]:
        ...
        "allow" not in entities["operations"] and "block" not in entities["operations"]
    ):
    ...
    if (
        "origin" not in entities
        and "destination" not in entities
        and not entities["targets"]
    ):
...
def build(entities):
    ...
    if "start" in entities and "end" in entities:
        if isinstance(entities["start"], datetime):
            entities["start"] = "{}:{}".format(
                entities["start"].hour, entities["start"].minute
            )
        ...
        intent += " start hour('{}') end hour('{}')".format(
