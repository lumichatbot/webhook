...
precision(data = factor(negation$prediction), reference = factor(negation$contradiction))
precision(data = factor(qos$prediction), reference = factor(qos$contradiction))
precision(data = factor(hierarchy$prediction), reference = factor(hierarchy$contradiction))
...
recall(data = factor(negation$prediction), reference = factor(negation$contradiction))
recall(data = factor(qos$prediction), reference = factor(qos$contradiction))
recall(data = factor(hierarchy$prediction), reference = factor(hierarchy$contradiction))
...
F_meas(data = factor(negation$prediction), reference = factor(negation$contradiction))
F_meas(data = factor(qos$prediction), reference = factor(qos$contradiction))
F_meas(data = factor(hierarchy$prediction), reference = factor(hierarchy$contradiction))
...
m + 
  #geom_bar(stat='identity', position='dodge') +
  geom_point(aes(y = precision, color = "Precision", shape = "Precision"), size = 4) +
  geom_point(aes(y = recall, color = "Recall", shape = "Recall"), size = 4) +
...
m + 
  #geom_bar(stat='identity', position='dodge') +
  geom_bar(aes(y = precision, color = "Precision")) +
  geom_bar(aes(y = recall, color = "Recall")) +
  #geom_line(aes(y = precision, color = "Precision", linetype = "Precision"), size = 1  ) + 
  #sgeom_line(aes(y = recall, color = "Recall", linetype = "Recall"), size =  1) +
  #geom_errorbar(aes(ymin=mean - se, ymax=mean + se), colour="black", width=0.1, position=position_dodge(0.8)) +
  #scale_x_continuous(limits = c(100,5000)) +
  scale_y_continuous(limits = c(0, 1), breaks=seq(0,1, by=0.1)) +
  xlab("# of Test Samples") + ylab("Metric") +
