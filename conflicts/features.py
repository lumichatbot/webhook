"""
    Module to extract conflicts features
        ...
        for hyp_qos in hypothesis_qos:
            if sentence_action == 'set' and hypothesis_action == 'set':
                if stn_qos['name'] == 'bandwidth' and hyp_qos['name'] == 'bandwidth':
                    # check individually
                    if not topology.is_bandwidth_available(sentence_path[0], sentence_path[1], stn_qos['value'], stn_qos['constraint']):
                        can_embed *= 0
                    elif not topology.is_bandwidth_available(hypothesis_path[0], hypothesis_path[1], hyp_qos['value'], hyp_qos['constraint']):
                        can_embed *= 0
                    # check combined
                        ...
                        can_embed *= 1 if topology.is_bandwidth_available(
                            common_path_src, common_path_tgt, stn_qos['value'] + hyp_qos['value'], hyp_qos['constraint']) else 0
                    else:
                        can_embed *= 1 if ((stn_qos['constraint'] == 'min' and stn_qos['value'] < hyp_qos['value']) or
                                           (stn_qos['constraint'] == 'max' and stn_qos['value'] > hyp_qos['value'])) else 0
                elif stn_qos['name'] == 'quota' and hyp_qos['name'] == 'quota':
                    if stn_qos['constraint'] == 'any' or hyp_qos['constraint'] == 'any':
                        can_embed *= 1 if stn_qos['value'] == hyp_qos['value'] and stn_qos['unit'] == hyp_qos['unit'] else 0
                    elif stn_qos['constraint'] == hyp_qos['constraint']:
                        can_embed *= 1 if stn_qos['value'] == hyp_qos['value'] and stn_qos['unit'] == hyp_qos['unit'] else 0
                    else:
                        can_embed *= 1
...
def hierarchy_targets(sentence, hypothesis):
    ...
    if stn_origin and stn_destination and hyp_origin and hyp_destination:
        ...
        descendent = topology.is_descendent(stn_destination, hyp_destination) or descendent
        # descendent = topology.is_descendent(hyp_origin, stn_origin) or descendent
        descendent = topology.is_descendent(hyp_destination, stn_destination) or descendent
    ...
    for traffic in stn_traffics:
        for service in hyp_services:
            ...
            ancestor = ancestor or next(
                (True for x in config.DATASET_TRAFFIC_ASSOCIATIONS[traffic]['services'] if x == service), False)

        for protocol in hyp_protocols:
            descendent = descendent or next(
                (True for x in config.DATASET_TRAFFIC_ASSOCIATIONS[traffic]['protocols'] if x == protocol), False)

    for service in stn_services:
        for protocol in hyp_protocols:
            descendent = descendent or next(
                (True for x in config.DATASET_SERVICE_ASSOCIATIONS[service]['protocols'] if x == protocol), False)

        for traffic in hyp_traffics:
            descendent = descendent or next(
                (True for x in config.DATASET_SERVICE_ASSOCIATIONS[service]['traffics'] if x == traffic), False)
...
def endpoints(sentence, hypothesis):
    ...
    for i in stn_endpoints:
        ...
        for j in hyp_endpoints:
            sim += endpoint.similarity(nlp(j))  # 1 if i == j else 0  # endpoint.similarity(nlp(j))
    return sim
...
def services(sentence, hypothesis):
    ...
    for i in stn_services:
        ...
        for j in hyp_services:
            sim += srv.similarity(nlp(j))  # 1 if i == j else 0  # srv.similarity(nlp(j))
...
def traffics(sentence, hypothesis):
    ...
    for i in stn_traffic:
        ...
        for j in hyp_traffic:
            sim += trf.similarity(nlp(j))  # 1 if i == j else 0  # trf.similarity(nlp(j))
...
def protocols(sentence, hypothesis):
    ...
    for i in stn_traffic:
        ptr = nlp(i)
        for j in hyp_traffic:
            sim += ptr.similarity(nlp(j))  # 1 if i == j else 0  # ptr.similarity(nlp(j))
...
def quotas(sentence, hypothesis):
    ...
    for i in stn_quota:
        quota = nlp(i)
        for j in hyp_quota:
            sim += quota.similarity(nlp(j))  # 1 if i == j else 0  # quota.similarity(nlp(j))
